...
def harrison_ford
  ...
  Movie.joins(:actors).where('castings.actor_id = 6 and castings.ord != 1').select('movies.id','movies.title')
...
def biggest_cast
  ...
  Movie.joins(:castings).group('movies.id','movies.title').order('COUNT(castings.id) DESC').limit(3).select('movies.id','movies.title')
end
...
def directed_by_one_of(them)
  ...
  Movie.select('movies.id','movies.title').joins(:director).where( {actors: {name: ['George Lucas', 'Steven Spielberg']}})
end
