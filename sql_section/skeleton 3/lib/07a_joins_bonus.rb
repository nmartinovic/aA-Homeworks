...
def expensive_tastes
  ...
  execute(<<-SQL)
  SELECT
  s.style, SUM(albums.price)/SUM(s.nick)
FROM
  albums
JOIN (
SELECT
  tracks.album, styles.style, COUNT(song) as nick
FROM
  tracks
JOIN
  styles on styles.album = tracks.album
GROUP BY tracks.album, styles.style) s ON s.album = albums.asin
GROUP BY s.style
HAVING SUM(albums.price)/SUM(s.nick) is not null
ORDER BY SUM(albums.price)/SUM(s.nick) DESC, style
LIMIT 5
  SQL
end
...
#  SELECT 
#     styles.style, COUNT(DISTINCT albums.title), COUNT(tracks.song), SUM(albums.price),(SUM(albums.price)/COUNT(tracks.song))/COUNT(tracks.song) as pps
#   FROM
#     albums
#   JOIN
#     styles on albums.asin = styles.album
#   JOIN
#     tracks on albums.asin = tracks.album 
#   WHERE styles.style = 'Styles > Classical > Forms & Genres > Symphonies > Romantic'
#   GROUP BY styles.style
#   HAVING (SUM(albums.price)/COUNT(tracks.song))/COUNT(tracks.song) is not null
#   ORDER BY pps DESC
#   LIMIT 5
...
# GROUP BY s.style
# HAVING SUM(albums.price)/SUM(s.nick) is not null
# ORDER BY SUM(albums.price)/SUM(s.nick) DESC
# LIMIT 5